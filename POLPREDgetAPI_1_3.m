function [PPTT,labels,units] = POLPREDgetAPI_1_3(params,fun)
...
%
% Optional:
% params.datum - datum, 'msl' or 'lat' 
%               (mean sea level or lowest astronomical tide)
...
%{
POLPREDgetAPI_dev.m - 1.3 (Matlab 2023a)
...
update history:
2024/10/25 DL: option for datum, msl or lat. fun = 'PL' and 'PTS' now works 
...
% these don't work
if ~exist('fun','var')
     error('POLPREDgetAPI: function not defined') 
end

if ~exist('params','var') && ~sum(strcmp(fun,'PL'))
     error('POLPREDgetAPI: no parameters defined') 
end
...
case 'PL'
% just so you don't get the error message
...
if ~strcmp(fun,'PL')
    % optional settings
    if ~sum(strcmp(fieldnames(params),'datum'))
        params.datum = 'msl'; % defaults to mean sea level
    end

    % Key
    if sum(strcmp(fieldnames(params),'key'))
        key = params.key; 
    else
...
url = [api,'compute-time-series?key=',key,'&lat=',lat,'&lng=',lon, ...
    '&dt_start=',sTime,'&dt_end=',eTime,'&interval=',int,'&datum=', ...
    params.datum];
...
case 'SA'
    ...
    %{
% testing --------------------------------
latN = '53.5'; latS = '49.5'; lonW = '-11.5'; lonE = '-2.5';
sTime = '2019-11-01T00:00:00Z';
% --
    %}

url = [api,'compute-spatial?key=',key,'&lat_n=',latN,'&lat_s=',latS, ...
    '&lng_w=',lonW,'&lng_e=',lonE,'&dt=',sTime,'&datum=', ...
    params.datum];
...
nVar = size(fields,1);
%{
&lat_n=53.5&lat_s=49.5&lng_w=-11.5&lng_e=-2.5&dt=2019-03-31T00:00:00Z
&lat_n=53.5&lat_s=49.5&lng_w=-11.5&lng_e=-2.5&dt=2019-11-01T00:00:00Z
&lat_n=51.9&lat_s=51.1&lng_w=-4.5&lng_e=-2.5&dt=2019-03-31T00:00:00Z
%}

% datetime  % PPTT.Time = ts.data.dt;
rowTimes = ...
